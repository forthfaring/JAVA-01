...
​	单台计算机设备中的应用程序可以通过进程id(pid)进行通信，但是在网络中就行不通了。TCP/TP协议就是为了解决这一问题，网络层的“ip地址”可以唯一表示网络中的主机，而传输层的“协议+端口”可以唯一标识主机中的进程。这样，利用协议、ip、端口就可以标识网络中的进程，这些进程就可以利用这个标识进行通信交互。
...
2、应用想读写数据需要数据在内核缓冲区和用户进程缓冲区之间来回拷贝数据，在等待数据准备好被拷贝的过程中，线程是阻塞的。
